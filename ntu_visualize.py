    ...
    def animate(skeleton):
        ...
        if anonymized:
            plt.title('Anonymized Skeleton {} Frame #{} of {} from {}\n (Action {}: {})\n{} ({})'.format(
                index, skeleton_index[0], skeleton_frames.shape[0], 'ntu', action_class, action_name, filename, 'male' if gender_class == 0 else 'female'),
                fontsize=8)
        else:
...
def visualize(data_path, action_label_path, gender_label_path, output_path, indices=[], filenames=[], anonymizer=None, weight=None, device=0, pool: Pool = None) -> MapResult:
    ...
    if anonymizer is None and weight:
        anonymizer = Anonymizer(
            num_point=25, num_person=2, num_gcn_scales=13, num_g3d_scales=6,
...
if __name__ == '__main__':
    ...
    visualize(args.datapath, args.labelpath,
              args.genderlabelpath, args.outputpath,
              indices=args.indices, weight=args.weight, device=args.device)
